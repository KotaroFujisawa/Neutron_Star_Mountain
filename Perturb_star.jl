...
using BoundaryValueDiffEq
    ...
    function cal_ξ_T(ρ, dρ_dr, d2ρ_dr2, cs2, dcs2_dr, 
...
#             println( 
#              3/r, " ",  + dcs2_dr(r)*3ρ(r)/(4μ(r)), " ", cs2(r)*(3ρ(r)/r)*3/4μ(r), " ", 
#              -cs2(r)*ρ(r)*dμ_dr(r)/μ(r)*3/4μ(r), " ",  + cs2(r)*dρ_dr(r)*3/(4μ(r))
#              )
            du[4] = (
                ρ(r) / r * δϕ(r) - ft(r) 
        ...
        println("ξra = $ξra, ξta = $ξta, T1a = $T1a, T2a = $T2a")
        fa = [ξra, ξta, T1a, T2a]
        p_a = ODEProblem(source, fa, rspan_a, param)
        sol_a  = solve(p_a, Tsit5(), reltol = 1.0e-8, abstol = 1.0e-8)

        #initial guess at r_o
        T2b = 0.0
        ξrb =  1.0e1
        ξtb = -1.0e1
        ...
        println("ξrb = $ξrb, ξtb = $ξtb, T1b = $T1b, T2b = $T2b")
        fb = [ξrb, ξtb, T1b, T2b]
        p_b = ODEProblem(source, fb, rspan_b, param)
        sol_b  = solve(p_b, Tsit5(), reltol = 1.0e-8, abstol = 1.0e-8)
        ...
        ξt = zeros(Float64, nr)
        for i = 1:nr
            if(r_c ≤ r_g[i] && r_g[i] ≤ r_m)
                ξr[i], ξt[i],  T1[i], T2[i] = sol_a(r_g[i])
            elseif(r_m ≤ r_g[i] && r_g[i] ≤ r_o)
                ξr[i], ξt[i],  T1[i], T2[i] = sol_b(r_g[i])
            else
                T1[i] = 0.0
                T2[i] = 0.0
                ξr[i] = 0.0
